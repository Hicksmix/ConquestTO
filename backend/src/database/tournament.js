...
async function createTournamentUser(userId, tournamentId, faction) {
    ...
    try {
        ...
        let row;
        if (conn) {
            let row = await conn.query('insert into `tournament_user` (user_id, tournament_id, faction) VALUES (?, ?, ?)', [userId, tournamentId, faction]);
            if (row.affectedRows === 1) return true;
        }
    } catch (e) {
        console.log(e);
